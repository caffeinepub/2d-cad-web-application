...
export default function PropertiesPanel({
  ...
  const isClosedShape =
    selectedObjectData &&
    ["rectangle", "circle", "ellipse", "octagon"].includes(
  ...
  }, [selectedObjectData, convertFromPixels]);
  ...
  const handleUpdate = (field: string, value: string) => {
    ...
    const pixelValue =
      field === "rotation" ? numValue : convertToPixels(numValue, unit);
  ...
  const handleMultiCopy = () => {
    const count = Number.parseInt(copyCount);
    const dx = Number.parseFloat(offsetX);
    const dy = Number.parseFloat(offsetY);
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-xs font-semibold">
              Position ({getUnitLabel()})
            </Label>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Label htmlFor="prop-x" className="text-xs">
                  X
                </Label>
                <Input
                  id="prop-x"
              ...
              <div>
                <Label htmlFor="prop-y" className="text-xs">
                  Y
                </Label>
                <Input
                  id="prop-y"
          ...
          {/* Rotation */}
          <div className="space-y-2">
            <Label htmlFor="prop-rotation" className="text-xs font-semibold">
              Rotation (degrees)
            </Label>
            <Input
              id="prop-rotation"
          ...
          {/* Color */}
          <div className="space-y-2">
            <Label htmlFor="prop-color" className="text-xs font-semibold">
              Stroke Color
            </Label>
            <Select value={color} onValueChange={handleColorChange}>
              ...
              <SelectContent>
                {COLOR_OPTIONS.filter((opt) => opt.value !== "none").map(
                  (option) => (
      ...
      <Dialog open={showMultiCopyDialog} onOpenChange={setShowMultiCopyDialog}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Create multiple copies of the selected object with specified
              offsets ({getUnitLabel()}).
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleMultiCopy}>Create Copies</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
