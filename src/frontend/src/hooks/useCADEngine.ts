...
import type { DrawingObject, Layer } from "../types/cad";
...
export function useCADEngine(
  ...
  const canvasRef =
    "current" in canvasRefOrProps
      ? canvasRefOrProps
      : canvasRefOrProps.canvasRef;
  const unit =
    "current" in canvasRefOrProps ? undefined : canvasRefOrProps.unit;
  const convertFromPixels =
    "current" in canvasRefOrProps
      ? undefined
      : canvasRefOrProps.convertFromPixels;
  const getUnitLabel =
    "current" in canvasRefOrProps ? undefined : canvasRefOrProps.getUnitLabel;
  ...
  const getAllEdges = (): EdgeInfo[] => {
    ...
    for (const obj of objects) {
      ...
      if (data.__kind__ === "line") {
        ...
        const octEdges = getOctagonEdges(
          data.octagon.octagon.center,
          data.octagon.octagon.size,
        );
  ...
  const explodeObjects = () => {
    ...
    for (const circleInfo of circles) {
      ...
      if (intersectionAngles.length > 0) {
        ...
        for (let i = 0; i < intersectionAngles.length; i++) {
          if (
            i === 0 ||
            Math.abs(intersectionAngles[i] - intersectionAngles[i - 1]) > 0.01
          ) {
    ...
    for (const obj of objects) {
      ...
      if (
        data.__kind__ === "line" ||
        data.__kind__ === "rectangle" ||
        data.__kind__ === "octagon" ||
        data.__kind__ === "polyline"
      ) {
        let objectEdges: Array<{ start: Point; end: Point }> = [];

        if (data.__kind__ === "line") {
          objectEdges = [
            { start: data.line.line.start, end: data.line.line.end },
          ];
        } else if (data.__kind__ === "rectangle") {
          objectEdges = getRectangleEdges(data.rectangle.rectangle);
        } else if (data.__kind__ === "octagon") {
          objectEdges = getOctagonEdges(
            data.octagon.octagon.center,
            data.octagon.octagon.size,
          );
        } else if (data.__kind__ === "polyline") {
        ...
        for (const edge of objectEdges) {
          ...
          for (const otherEdge of allEdges) {
            ...
            if (intersection) {
              ...
              const isAtStart =
                Math.abs(intersection.x - edge.start.x) < 0.01 &&
                Math.abs(intersection.y - edge.start.y) < 0.01;
              const isAtEnd =
                Math.abs(intersection.x - edge.end.x) < 0.01 &&
          ...
          if (intersectionPoints.length > 0) {
            const sortedPoints = [
              ...
              const distA = Math.sqrt(
                (a.x - edge.start.x) ** 2 + (a.y - edge.start.y) ** 2,
              );
              const distB = Math.sqrt(
                (b.x - edge.start.x) ** 2 + (b.y - edge.start.y) ** 2,
              );
              return distA - distB;
            });
    ...
    for (const obj of objects) {
      ...
      if (data.__kind__ === "line") {
        ...
        if (centerDist < closestDistance) {
          closestDistance = centerDist;
          closestSnap = {
            point: data.circle.circle.center,
            type: "circle-center",
          };
        }
        ...
        if (centerDist < closestDistance) {
          closestDistance = centerDist;
          closestSnap = {
            point: data.ellipse.ellipse.center,
            type: "circle-center",
          };
        }
      } else if (data.__kind__ === "rectangle") {
        ...
        const edgeMidpoints = [
          {
            x:
              data.rectangle.rectangle.topLeft.x +
              data.rectangle.rectangle.width / 2,
            y: data.rectangle.rectangle.topLeft.y,
          },
          {
            x:
            ...
            y:
              data.rectangle.rectangle.topLeft.y +
              data.rectangle.rectangle.height / 2,
          },
          {
            x:
              data.rectangle.rectangle.topLeft.x +
        ...
        for (const midpoint of edgeMidpoints) {
          const dist = Math.sqrt(
            (point.x - midpoint.x) ** 2 + (point.y - midpoint.y) ** 2,
          );
          if (dist < closestDistance) {
            closestDistance = dist;
        ...
        // Octagon corners
        const corners = getOctagonCorners(
          data.octagon.octagon.center,
          data.octagon.octagon.size,
        );
        for (const corner of corners) {
  ...
  const loadProjectData = (
    projectObjects: DrawingObject[],
    projectLayers?: Layer[],
    projectActiveLayer?: number,
  ) => {
  ...
  const findObjectAtPoint = (point: Point): string | null => {
    for (let i = objects.length - 1; i >= 0; i--) {
      ...
      if (data.__kind__ === "line") {
        ...
        const dx =
          (point.x - data.ellipse.ellipse.center.x) /
          data.ellipse.ellipse.radiusX;
        const dy =
          (point.y - data.ellipse.ellipse.center.y) /
        ...
        if (
          ...
          5 /
            zoom /
            Math.max(data.ellipse.ellipse.radiusX, data.ellipse.ellipse.radiusY)
        ) {
          return obj.id;
        }
      } else if (data.__kind__ === "rectangle") {
  ...
  const updateObjectProperties = (
    ...
    const updatedObjects = objects.map((obj) => {
      ...
      if (data.__kind__ === "line") {
        const dx =
          updates.x !== undefined ? updates.x - data.line.line.start.x : 0;
        const dy =
          updates.y !== undefined ? updates.y - data.line.line.start.y : 0;
        newData = {
          __kind__: "line",
          line: {
            line: {
              start: {
                x: data.line.line.start.x + dx,
                y: data.line.line.start.y + dy,
              },
              end: {
                x: data.line.line.end.x + dx,
                y: data.line.line.end.y + dy,
              },
            },
            color: updates.color ? hexToColor(updates.color) : data.line.color,
            rotation:
              updates.rotation !== undefined
                ? (updates.rotation * Math.PI) / 180
                : data.line.rotation,
            layer: BigInt(obj.layerId),
          },
        };
        ...
        newData = {
          ...
          circle: {
            circle: {
              center: {
                x:
                  updates.x !== undefined
                    ? updates.x
                    : data.circle.circle.center.x,
                y:
                  updates.y !== undefined
                    ? updates.y
                    : data.circle.circle.center.y,
              },
              radius:
            ...
            color: updates.color
              ? hexToColor(updates.color)
              : data.circle.color,
            rotation:
              updates.rotation !== undefined
                ? (updates.rotation * Math.PI) / 180
                : data.circle.rotation,
            layer: BigInt(obj.layerId),
            fill: updates.fillColor
              ? hexToColor(updates.fillColor)
              : data.circle.fill,
          },
        };
      } else if (data.__kind__ === "ellipse") {
        ...
        newData = {
          ...
          rectangle: {
            ...
            color: updates.color
              ? hexToColor(updates.color)
              : data.rectangle.color,
            rotation:
              updates.rotation !== undefined
                ? (updates.rotation * Math.PI) / 180
                : data.rectangle.rotation,
            layer: BigInt(obj.layerId),
            fill: updates.fillColor
              ? hexToColor(updates.fillColor)
              : data.rectangle.fill,
            ...
            octagon: {
              center: {
                x:
                  updates.x !== undefined
                    ? updates.x
                    : data.octagon.octagon.center.x,
                y:
                  updates.y !== undefined
                    ? updates.y
                    : data.octagon.octagon.center.y,
              },
              size:
                updates.width !== undefined
                  ? updates.width / 2
        ...
        if (axis === "horizontal") {
          newData = {
            ...
            line: {
              line: {
                start: {
                  x: 2 * centerX - data.line.line.start.x,
                  y: data.line.line.start.y,
                },
                end: {
                  x: 2 * centerX - data.line.line.end.x,
                  y: data.line.line.end.y,
                },
              },
              color: data.line.color,
              rotation: data.line.rotation,
            ...
            line: {
              line: {
                start: {
                  ...
                  y: 2 * centerY - data.line.line.start.y,
                },
                end: {
                  x: data.line.line.end.x,
                  y: 2 * centerY - data.line.line.end.y,
                },
              },
              color: data.line.color,
              rotation: data.line.rotation,
        ...
        if (data.__kind__ === "line") {
          ...
          newData = {
            ...
            circle: {
              circle: {
                center: {
                  x: data.circle.circle.center.x + dx,
                  y: data.circle.circle.center.y + dy,
                },
                radius: data.circle.circle.radius,
              },
              color: data.circle.color,
          ...
          newData = {
            ...
            ellipse: {
              ellipse: {
                center: {
                  x: data.ellipse.ellipse.center.x + dx,
                  y: data.ellipse.ellipse.center.y + dy,
                },
          ...
          newData = {
            ...
            rectangle: {
              rectangle: {
                topLeft: {
                  x: data.rectangle.rectangle.topLeft.x + dx,
                  y: data.rectangle.rectangle.topLeft.y + dy,
                },
                width: data.rectangle.rectangle.width,
                height: data.rectangle.rectangle.height,
              },
          ...
          newData = {
            ...
            octagon: {
              octagon: {
                center: {
                  x: data.octagon.octagon.center.x + dx,
                  y: data.octagon.octagon.center.y + dy,
                },
          ...
          newData = {
            ...
            polyline: {
              polyline: {
                points: data.polyline.polyline.points.map((p) => ({
                  x: p.x + dx,
                  y: p.y + dy,
                })),
              },
          ...
          newData = {
            ...
            arc: {
              arc: {
                center: {
                  x: data.arc.arc.center.x + dx,
                  y: data.arc.arc.center.y + dy,
                },
    ...
    if (state.isRectangleSelecting && state.rectangleSelectStart) {
      ...
      const updatedObjects = objects.map((obj) => {
        ...
        if (tool === "move" || tool === "copy") {
          if (originalData.__kind__ === "line") {
            newData = {
              ...
              line: {
                line: {
                  start: {
                    x: originalData.line.line.start.x + dx,
                    y: originalData.line.line.start.y + dy,
                  },
                  end: {
                    x: originalData.line.line.end.x + dx,
                    y: originalData.line.line.end.y + dy,
                  },
                },
            ...
            newData = {
              ...
              circle: {
                circle: {
                  center: {
                    x: originalData.circle.circle.center.x + dx,
                    y: originalData.circle.circle.center.y + dy,
                  },
            ...
            newData = {
              ...
              ellipse: {
                ellipse: {
                  center: {
                    x: originalData.ellipse.ellipse.center.x + dx,
                    y: originalData.ellipse.ellipse.center.y + dy,
                  },
            ...
            newData = {
              ...
              rectangle: {
                rectangle: {
                  topLeft: {
                    x: originalData.rectangle.rectangle.topLeft.x + dx,
                    y: originalData.rectangle.rectangle.topLeft.y + dy,
                  },
            ...
            newData = {
              ...
              octagon: {
                octagon: {
                  center: {
                    x: originalData.octagon.octagon.center.x + dx,
                    y: originalData.octagon.octagon.center.y + dy,
                  },
            ...
            newData = {
              ...
              polyline: {
                polyline: {
                  points: originalData.polyline.polyline.points.map((p) => ({
                    x: p.x + dx,
                    y: p.y + dy,
                  })),
                },
            ...
            newData = {
              ...
              arc: {
                arc: {
                  center: {
                    x: originalData.arc.arc.center.x + dx,
                    y: originalData.arc.arc.center.y + dy,
                  },
          ...
          if (originalData.__kind__ === "line") {
            const centerX =
              (originalData.line.line.start.x + originalData.line.line.end.x) /
              2;
            const centerY =
              (originalData.line.line.start.y + originalData.line.line.end.y) /
              2;
            const rotatePoint = (p: Point) => ({
              x:
                centerX +
                (p.x - centerX) * Math.cos(angle) -
                (p.y - centerY) * Math.sin(angle),
              y:
                centerY +
                (p.x - centerX) * Math.sin(angle) +
                (p.y - centerY) * Math.cos(angle),
            });
            newData = {
              __kind__: "line",
              line: {
  ...
  const renderCanvas = () => {
    ...
    for (const obj of objects) {
      ...
      if (
        fillColor !== "none" &&
        (data.__kind__ === "circle" ||
          data.__kind__ === "ellipse" ||
          data.__kind__ === "rectangle" ||
          data.__kind__ === "octagon")
      ) {
    ...
    if (
      state.isRectangleSelecting &&
      state.rectangleSelectStart &&
      state.currentPoint
    ) {
    ...
    if (
      tool === "arcedit" &&
      state.arcEditState.isDefiningArc &&
      state.arcEditState.selectedEdge &&
      state.currentPoint
    ) {
      const edge = state.arcEditState.selectedEdge;
      const start = edge.start;
    ...
    if (tool === "measure" && state.measurementPoints.length > 0) {
      ...
      if (state.measurementPoints.length === 1 && state.currentPoint) {
        ...
        // Convert distance to selected unit
        const displayValue = convertFromPixels ? convertFromPixels(dist) : dist;
        const unitLabel = getUnitLabel ? getUnitLabel() : "px";
        ...
        const angle1 = Math.atan2(
          state.measurementPoints[0].y - state.measurementPoints[1].y,
          state.measurementPoints[0].x - state.measurementPoints[1].x,
        );
        const angle2 = Math.atan2(
          state.currentPoint.y - state.measurementPoints[1].y,
          state.currentPoint.x - state.measurementPoints[1].x,
        );
        ctx.beginPath();
        ctx.arc(
          state.measurementPoints[1].x,
          state.measurementPoints[1].y,
          radius,
          angle1,
          angle2,
        );
        ...
        ctx.fillText(
          `${angle.toFixed(2)}°`,
          state.measurementPoints[1].x + radius,
          state.measurementPoints[1].y,
        );
      }
      ...
      for (const p of state.measurementPoints) {
        ctx.beginPath();
    ...
    if (state.snapPoint) {
      ...
      ctx.fillText(
        state.snapPoint.type,
        state.snapPoint.point.x + 10 / zoom,
        state.snapPoint.point.y - 10 / zoom,
      );
    }
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: inline functions recreate each render; intentional
  useEffect(() => {
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: functions defined inline recreate each render
  }, [selectedObjectIds, objects, tool]);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: renderCanvas is inline; object deps trigger re-render
  useEffect(() => {
    renderCanvas();
  }, [
    objects,
